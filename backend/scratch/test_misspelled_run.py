...
async def main():
    async with httpx.AsyncClient(timeout=120.0) as client:
        payload = {
            "location": {
                "raw_text": "ameerpat, hyderabad",
                "locality": "ameerpat",
                "city": "hyderabad",
                "country": "India"
            },
            "keywords": ["degree collage"],
            "max_results": 25,
            "min_confidence": 0.5,
        ...
        print("Submitting run for 'degree collage' (misspelled) in 'ameerpat, hyderabad'...")
        ...
        while time.time() - start_t < 90:
            ...
            if status in ("completed", "failed", "partial"):
                ...
                print(f"\nAccepted Leads Count for misspelled input: {len(accepted_leads)}")
                for idx, lead in enumerate(accepted_leads[:10], 1):
                    name = lead.get("canonical_name")
                    tier = lead.get("tier")
                    print(f"  {idx}. {name} | Tier: {tier}")
                break
